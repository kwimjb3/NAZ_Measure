...
def rank_matches(
    ...
    dist_map = pd.Series(dist[t_idx], index=control_ids)

    ranked = control_scaled.copy()
    ranked["Distance"] = ranked["VPID"].map(dist_map).fillna(np.inf)
    ranked = ranked.sort_values("Distance").reset_index(drop=True)
    ranked["Rank"] = range(1, len(ranked) + 1)
    return ranked


def stable_match_vpid(
    vpid: int,
...
def pre_post_true_trend_chart(trends_df: pd.DataFrame, *, title: str) -> go.Figure | None:
    ...
    def add_step_arrow(col: int, pre: float, post: float):
        ...
        fig.add_shape(
            type="line", xref=f"x{col}", yref="y",
            x0=x_pre, y0=pre_top, x1=x_pre, y1=y_top,
            line=dict(color="#111111", width=2),
        )
        fig.add_shape(
            type="line", xref=f"x{col}", yref="y",
            x0=x_pre, y0=y_top, x1=x_post, y1=y_top,
            line=dict(color="#111111", width=2),
        )
        fig.add_annotation(
            x=x_post, y=post_top, xref=f"x{col}", yref="y",
            ax=x_post, ay=y_top, axref=f"x{col}", ayref="y",
            showarrow=True, arrowhead=2, arrowsize=1.0,
            arrowwidth=2, arrowcolor="#111111", text="",
        )
        delta_pts = (post - pre) * 100.0
        xdom = "x domain" if col == 1 else "x2 domain"
        fig.add_annotation(
            x=0.5, y=y_top, xref=xdom, yref="y",
            text=f"{delta_pts:+.1f}", showarrow=False,
            font=dict(size=14, color="#111111"),
            bgcolor="white", bordercolor="#111111",
            borderwidth=2, borderpad=6, yshift=14,
        )
    ...
    fig.update_yaxes(
        title_text="YoY Trend (true group)", tickformat=".0%",
        ...
        with tabs[i]:
            ...
            for k in offsets_local:
                ...
                if (
                    ...
                    fig = pre_post_true_trend_chart(
                        trends_df, title=f"YoY Trend Shift — {cfg_name} (offset={k})"
                    )
                    if fig is not None:
        ...
        if selected_vpid is not None:
            ...
            for cfg_name in selected_cfg_names:
                ...
                for k in offsets_for_cfg:
                    ...
                    if (
                        cand is not None
                        and not getattr(cand, "empty", False)
                        and selected_vpid in cand["Test_VPID"].values
                    ):
                        df_pairs = cand
                        chosen_offset = k
                        break
                ...
                stable_match = stable_match_vpid(selected_vpid, pre_df, pre_df, cfg)
                ...
                if not ranked.empty:
                    st.caption(f"Top-ranked control (min distance) → {ranked.iloc[0]['VPID']}")

                if actual_job_match is not None and not ranked.empty:
                    if actual_job_match != ranked.iloc[0]["VPID"]:
                        st.warning(
